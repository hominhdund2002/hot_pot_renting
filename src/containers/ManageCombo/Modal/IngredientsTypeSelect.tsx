...
const IngredientsTypeSelect: React.FC<IngredientsTypeSelectorProps> = ({
  open,
  ...
  // Handle checkbox selection - FIXED TYPE ERROR
  const handleIngredientChange = (ingredient: IngredientType) => {
    setSelectedIngredientTypes((prev) => {
      ...
      if (isSelected) {
        ...
        // FIX: Properly map IngredientType to SelectedIngredientType
        const newSelectedIngredient: SelectedIngredientType = {
          ingredientTypeId: ingredient.ingredientTypeId,
          ingredientTypeName: ingredient.name, // Map 'name' to 'ingredientTypeName'
          ingredientCount: ingredient.ingredientCount,
          quantity: 1,
          minQuantity: 0,
        };

        return [...prev, newSelectedIngredient];
      }
    });
  };
    ...
    selectedIngredientTypes.forEach((selectedType) => {
      ...
      for (let i = 0; i < selectedType.quantity; i++) {
        result.push({
          ...
          name: selectedType.ingredientTypeName, // Map back to 'name' from 'ingredientTypeName'
...
export default IngredientsTypeSelect;
